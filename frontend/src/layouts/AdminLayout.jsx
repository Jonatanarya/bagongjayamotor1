...
const navItems = [
  ...
  { to: '/admin/stok', icon: 'two_wheeler', label: 'Stok Motor' },
  { to: '/admin/transaksi', icon: 'receipt_long', label: 'Transaksi' },
  { to: '/admin/request', icon: 'inbox', label: 'Request Pembelian' },
  { to: '/admin/laporan', icon: 'bar_chart', label: 'Laporan Excel' },
  { to: '/admin/saran', icon: 'forum', label: 'Saran Customer' },
];
...
function AdminLayout({ children }) {
  ...
  return (
    <div className="flex h-screen bg-slate-900 overflow-hidden">
      {/* Desktop Sidebar */}
      <aside
        className={`hidden md:flex flex-col bg-slate-950 border-r border-white/5 transition-all duration-200 shrink-0 ${collapsed ? 'w-16' : 'w-60'}`}
      >
        <SidebarContent />
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="bg-slate-900 border-b border-white/5 px-6 py-4 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-4">
            {/* Mobile menu toggle */}
            <button
              className="md:hidden cursor-pointer text-slate-400 hover:text-white"
            ...
            <button
              className="hidden md:block cursor-pointer text-slate-400 hover:text-white transition-colors"
