...
import { sellRequests } from './schema/sellRequests';
...
async function seed() {
  ...
  try {
    ...
    const existingAdmin = await db.select().from(users).where(eq(users.email, 'admin@bagongjaya.com')).limit(1);
    ...
    const existingMotors = await db.select().from(motors).limit(1);
    ...
    const existingSuggestions = await db.select().from(suggestions).limit(1);
    ...
    const existingSellRequests = await db.select().from(sellRequests).limit(1);
    if (existingSellRequests.length === 0) {
      // Seed sample sell requests
      console.log('Creating sample sell requests...');
      await db.insert(sellRequests).values([
        {
          id: 'sell-request-1',
